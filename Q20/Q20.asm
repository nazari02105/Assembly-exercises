...
DtSeg   Segment
        ...
        temp DW 0;temp word
        complete DB 10,13, 'complete',10,13,'$'
        notComplete DB 10,13, 'not complete',10,13,'$'
        functionArgument DW ?;this is function argument
        swap DW ?
        functionResult DB ?;this is function result
DtSeg   ENDS
...
MOV DX, OFFSET notComplete; we consider user input is not complete at first
CALL getInt;using q1 function :>>
MOV AX, number
MOV functionArgument, AX;for giving function argument
CALL isComplete;check if it is complete number or not
MOV AL, functionResult
CMP AL, 0;if number is complete, then AL will be one so if it is zero, it means that number is not complete
JZ all;if AL is zero then we should print "notComplete" sentence
MOV DX, OFFSET complete;otherwise we should load "complete" sentence to print 
all:
MOV AH, 09H;for syscall
INT 21H;for syscall
MOV AX, 0;counter for zero to 255
mainLoop:
INC AX;for starting from 1
CMP AL, 0;because of overflows which appeared when we move to 256 from 255 and because AL is 8 bits
JZ outerFinish;if AL is zero, it means we should break loop
MOV swap, AX
MOV functionArgument, AX
CALL isComplete
MOV AL, functionResult
MOV BX, swap
CMP AL, 0;it means answer is not complete so we should not print the number
JZ notPrint
MOV number, BX;for printing
MOV swap, BX
CALL printInt;using q1 functions :>>
MOV BX, swap
notPrint:
...
isComplete PROC NEAR:
MOV AX, functionArgument
MOV AH, 0;it is positive
MOV BX, 0;sum
MOV CX, 0;counter
completeLoop:
MOV AX, functionArgument
MOV AH, 0;it is positive
INC CX
CMP CL, AL;from 255 to 256 an overflow aquired
JZ completeFinish;finish loop
JMP checkZero
completeFinish:
MOV AX, functionArgument
MOV AH, 0;it is positive        
...
MOV DL, 1
mainMainFinish:
MOV functionResult, DL
 RET
checkZero:
MOV AH, 0;it is positive
DIV CL;div for mod
CMP AH, 0;Check factor
JNZ completeLoop;if remainder is not zero, we shoud continue loop
ADD BX, CX;now CX is factor of BX so we should add it to BX
JMP completeLoop
isComplete ENDP                 
